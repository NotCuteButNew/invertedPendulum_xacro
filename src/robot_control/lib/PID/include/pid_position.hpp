...
namespace pid {
template <typename T> class PositionalPid : public PidInterface<T> {
private:
  T kp_;
  T ki_;
  T kd_;
...
public:
  ...
  PositionalPid<T>(T p, T i, T d, bool enable = false, T min = 0, T max = 0)
...
template <typename T>
inline bool PositionalPid<T>::SetPid(const T p, const T i, const T d) {
  kp_ = p;
  ki_ = i;
  kd_ = d;
  return true;
...
template <typename T>
inline bool PositionalPid<T>::SetLimiterStatus(bool status) {
  enable_limit_ = status;
  return enable_limit_ == status;
}

template <typename T> inline bool PositionalPid<T>::SetLimits(T min, T max) {
  limit_min_ = min;
  limit_max_ = max;
  return true;
}

template <typename T> T PositionalPid<T>::Proportional() { return e_; }

template <typename T> T PositionalPid<T>::Integral() {
  return integral_0_t_ += e_;
}

template <typename T> T PositionalPid<T>::Derivative() { return e_ - last_e_; }

template <typename T>
T PositionalPid<T>::Calculate(const T target, const T actual) {
  e_ = target - actual;
